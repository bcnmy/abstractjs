...
async function getErc20CoinsByMarketCap(limit = 100): Promise<string[]> {
...
async function generateTokenConstants(coins: FinalisedCoin[]) {
  const startOfFile = `import { getMultichainContract } from "../../utils/contract/getMultichainContract";\nimport { erc20Abi } from "viem"\n\n`
  ...
  writeTsToFile(
    `${startOfFile}\n\n${tokenConstants}`,
    path.join(__dirname, "../src/commons/tokens/index.ts")
  )
}

async function main() {
  const coinIds = await getErc20CoinsByMarketCap()
