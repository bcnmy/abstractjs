...
export class MeeService {
  ...
  constructor(params: MeeServiceInitParams) {
    this.nodeUrl = params.meeNodeUrl;
  }
  ...
  async getQuote({
    ...
    const userOpData = await Promise.all(
      userOps.map(async (userOp) => {
        ...
        if (!deployment) {
          throw Error(
            `No account deployment found for chain ${userOp.chainId}`
          );
        }
    ...
    var paymentInfoData: PaymentInfo = (await paymentAccount.isDeployed())
      ? {
          ...paymentInfoBaseData,
        }
      : {
          ...paymentInfoBaseData,
          initCode: await paymentAccount.getInitCode(),
        };
  ...
  async execute(parameters: {
    ...
    if (!response.ok) {
      const error = await response.text();
      throw new Error(`Failed to execute transaction through the MEE Node: ${error}`);
    }
